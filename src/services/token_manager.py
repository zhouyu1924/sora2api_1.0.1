...
class TokenManager:
    ...
    async def activate_sora2_invite(self, access_token: str, invite_code: str) -> dict:
        ...
        async with AsyncSession() as session:
            ...
            kwargs = {
                ...
                "timeout": 30,
                "impersonate": "safari_ios"  # 使用 chrome120 让库自动处理 UA 等头
            }
    ...
    async def add_token(self, token_value: str,
        ...
        if skip_status_update:
            ...
            # Process User Info
            if isinstance(user_info_res, Exception):
                # If API call fails, use JWT data
                email = jwt_email or ""
                name = email.split("@")[0] if email else ""
                print(f"Failed to get user info: {user_info_res}")
            else:
                email = user_info_res.get("email", jwt_email or "")
                name = user_info_res.get("name") or ""

            # Process Subscription Info
            if isinstance(sub_info_res, Exception):
                error_msg = str(sub_info_res)
                # Re-raise if it's a critical error (token expired)
                if "Token已过期" in error_msg:
                    raise sub_info_res
                # If API call fails, subscription info will be None
                print(f"Failed to get subscription info: {sub_info_res}")
            else:
                sub_info = sub_info_res
            ...
            if isinstance(sora2_info_res, Exception):
                error_msg = str(sora2_info_res)
                # Re-raise if it's a critical error (unsupported country)
                if "Sora在您的国家/地区不可用" in error_msg:
                    raise sora2_info_res
                print(f"Failed to get Sora2 info: {sora2_info_res}")
            else:
                sora2_info = sora2_info_res
